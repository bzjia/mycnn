function convolvedFeatures = cnnConvolve(filterDim, inputMaps,outputMaps, inputFeatures, W, b)
...
numImages = size(inputFeatures, 4);
imageDim = size(inputFeatures, 1);
convDim = imageDim - filterDim + 1;

convolvedFeatures = zeros(convDim, convDim, outputMaps, numImages);
...
%   for filterNum = 1:outputMaps
%       for inputMapN = 1:inputMaps
% 
...
%       
%     % Obtain the image
%             im = squeeze(inputFeatures(:, :,inputMapN, imageNum));
% 
...
for j = 1:outputMaps
    convolvedImage = zeros(convDim, convDim,numImages);
    for i = 1:inputMaps
        convolvedImage = convolvedImage + convn(squeeze(inputFeatures(:,:,i,:)),...
            squeeze(W(:,:,i,j)),'valid');
    end
    convolvedFeatures(:,:,j,:) = sigmoid(convolvedImage+b(j));
end
